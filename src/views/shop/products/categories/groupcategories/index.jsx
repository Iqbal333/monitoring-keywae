...
import { getCategoryGroup } from '@/api/shops/products';
import {
  Card,
  Table,
  Button,
...
class GroupCategoriesProducts extends Component {
  ...
  fetchData = () => {
    ...
    getCategoryGroup(this.props.match.params.id, this.state.listQuery, this.state.filter).then((response) => {
  ...
  render() {
    const columns = [
      {
        title: 'Nama Grup Kategori',
        dataIndex: 'name',
        key: 'name',
      },
      {
        title: 'Action',
        dataIndex: 'productgroup_id',
        key: 'action',
        render: (_, record) => (
          <Space size={'middle'}>
            <Link to={'/shop/categories/group-type/'+_}>
    ...
    return (
      <div className='app-container'>
        ...
        <br />
        <Card title='DAFTAR GRUP KATEGORI PRODUK'>
          <Table
            size='small'
