...
import { getCategory } from '@/api/shops/products';
import {
  Card,
  Table,
  Button,
...
class CategoriesProducts extends Component {
  ...
  fetchData = () => {
    ...
    getCategory(this.state.listQuery, this.state.filter).then((response) => {
  ...
  render() {
    const columns = [
        ...
        render: (_, { icon }) => (
          <>
            <Avatar
              shape='square'
              size={35}
              src={icon !== null && icon !== '' ? icon : noImage}
            />
          </>
        ),
      },
        ...
        render: (tag) => (
          <>
            <Tag color={tag === 'false' ? 'magenta' : 'green'} key={tag}>
              {tag === 'false' ? 'TIDAK DIFITURKAN' : 'DIFITURKAN'}
            </Tag>
          </>
        ),
      },
      ...
      },
      {
        ...
        render: (_, record) => (
          <Space size={'middle'}>
            <Link to={'/shop/categories/group/'+_}>
    ...
    return (
      <div className='app-container'>
        ...
        <br />
        <Card title='DAFTAR KATEGORI PRODUK'>
          <Table
            size='small'
