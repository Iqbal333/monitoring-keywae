...
import { getCanceled } from '@/api/shops/orders';
...
class CanceledOrders extends Component {
  ...
  fetchData = () => {
    ...
    getCanceled(this.state.listQuery, this.state.filter).then((response) => {
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        render: (tag) => (
          <>
            <Tag color={tag === 'Dibatalkan' ? 'magenta' : 'green'} key={tag}>
              {tag === 'Dibatalkan' ? tag : '-'}
            </Tag>
          </>
        ),
